...
function updateDepartmentByID(
    ...
    $db = connectionDb();
    $checkUpdate = false;
    $sql = "UPDATE `departments` SET `Name` = :nameDepartment, `Slug` = :slug, `Leader` = :leader,  `date_beginning` = :date_beginning, `Status` = :statusDepartment, `logo` = :logo, `Updated_at` = :update_at WHERE `ID` = :id AND `Deleted_at` IS NULL";
